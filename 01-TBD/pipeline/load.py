...
from db.archive_schema import player_gw_history
...
def upsert_gameweeks(engine, gameweek_data: list[dict], season_id: int):
    ...
    stmt = stmt.on_conflict_do_update(
        set_={col: stmt.excluded[col] for col in cleaned.keys()}
    )
...
def upsert_future_fixtures(engine, fixture_data: list[dict], fetched_gameweek_id: int):
    cleaned = [clean_future_fixture(row, fetched_gameweek_id) for row in fixture_data]
...
def upsert_gw_history(engine, player_id: int, season_id: int, opta_code: str, history_data: list[dict]):
    ...
    stmt = stmt.on_conflict_do_update(
        constraint="uq_history_player_fixture",
        set_={col: stmt.excluded[col] for col in cleaned[0].keys()}
    )
    with engine.begin() as conn:
        conn.execute(stmt)